...
('bubbleSortArrFunction %s', ({sortDirection, result}) => {
  ...
  test('Empty array', async () => {
    const outputArray = await bubbleSort([], setLoader, setArray, sortDirection === 'inc');
    expect(outputArray).toEqual([]);
  });
  ...
  test('Array with multiple elements', async () => {
    const outputArray = await bubbleSort(inputArr, setLoader, setArray, sortDirection === 'inc');
    expect(outputArray).toEqual(result);
  });
